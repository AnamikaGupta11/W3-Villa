...
const InputData = ({ InputDiv, setInputDiv, UpdatedData, setUpdatedData }) => {
  ...
  const submitData = async () => {
    if (Data.title === "" || Data.desc === "") {
      ...
      await axios.post("https://task-manager-backend-7doy.onrender.com/api/v2/create-task", Data, {
        headers,
      });
      setData({ title: "", desc: "" });
      setInputDiv("hidden");
  ...
  const UpdateTask = async () => {
    if (Data.title === "" || Data.desc === "") {
      ...
      await axios.put(
        `https://task-manager-backend-7doy.onrender.com/api/v2/update-task/${UpdatedData.id}`,
        Data,
        {
          headers,
        }
